import { Component } from '@angular/core';

@Component({
  selector: 'app-address',
  templateUrl: './address.component.html',
  styleUrls: ['./address.component.css'],
})
export class AddressComponent {
  firstLevelArr = [
    { id: '1', name: 'India' },
    { id: '2', name: 'Germany' },
  ];
  secondLevelArr = [
    { id: 's1', parentId: '2', name: 'Bavaria' },
    { id: 's2', parentId: '2', name: 'Berlin' },
    { id: 's3', parentId: '1', name: 'Maharashtra' },
    { id: 's4', parentId: '1', name: 'Tamilnadu' },
  ];
  thirdLevelArr = [
    { id: 'd1', parentId: 's1', name: 'Upper Bavaria' },
    { id: 'd2', parentId: 's1', name: 'Lower Bavaria' },
    { id: 'd3', parentId: 's2', name: 'Berlin-Mitte' },
    { id: 'd4', parentId: 's2', name: 'Kreuzberg' },
    { id: 'd5', parentId: 's3', name: 'Nashik' },
    { id: 'd6', parentId: 's3', name: 'Jalgoan' },
    { id: 'd7', parentId: 's4', name: 'Ariyalur' },
    { id: 'd8', parentId: 's4', name: 'Chennai' },
  ];
  fourthLevelArr = [
    { id: 'p1', parentId: 'd1', name: 'Munich' },
    { id: 'p2', parentId: 'd1', name: 'Erding' },
    { id: 'p3', parentId: 'd2', name: 'Leipzig' },
    { id: 'p4', parentId: 'd2', name: 'Landshut' },
    { id: 'p5', parentId: 'd3', name: 'Passau' },
    { id: 'p6', parentId: 'd3', name: 'Gesundbrunnen' },
    { id: 'p7', parentId: 'd4', name: 'Frieburg' },
    { id: 'p8', parentId: 'd4', name: 'Hamburg' },
    { id: 'p9', parentId: 'd6', name: 'Raver' },
    { id: 'p10', parentId: 'd6', name: 'Savda' },
    { id: 'p11', parentId: 'd5', name: 'Ozar' },
    { id: 'p12', parentId: 'd5', name: 'Manmad' },
    { id: 'p13', parentId: 'd7', name: 'Thirumanur' },
    { id: 'p14', parentId: 'd7', name: 'Sendurai' },
    { id: 'p15', parentId: 'd8', name: 'New Chennai' },
    { id: 'p16', parentId: 'd8', name: 'Old Chennai' },
  ];

  constructor() {}

  ngOnInit(): void {
    let finalObj: any = {
      countries: {},
    };

    let stateCounter: number = 0;
    let districtsCounter: number = 0;
    let placeCounter: number = 0;

    this.firstLevelArr.forEach((el: any, ind: number) => {
      let index = '0' + (ind + 1);
      finalObj.countries[index] = {};
      finalObj.countries[index]['countryName'] = el.name;

      if (!finalObj.countries[index]['states']) {
        finalObj.countries[index]['states'] = {};
      }
      let filterState = this.secondLevelArr.filter(
        (st) => st.parentId == el.id
      )

      filterState.forEach((fs: any) => {
        ++stateCounter;
        finalObj.countries[index]['states']['S' + stateCounter] = {};
        finalObj.countries[index]['states']['S' + stateCounter]['stateName'] =
          fs.name;
      
        let filterDistrics = this.thirdLevelArr.filter(
          (districts) => districts.parentId == fs.id
        );
        filterDistrics.forEach((fd: any) => {
          ++districtsCounter;
          finalObj.countries[index]['states']['S' + stateCounter][
            'd' + districtsCounter
          ] = {};
          finalObj.countries[index]['states']['S' + stateCounter][
            'd' + districtsCounter
          ]['districtName'] = fd.name;
          let filterPlaces = this.fourthLevelArr.filter(
            (places) => places.parentId == fd.id
          );
          filterPlaces.forEach((fp: any) => {
            ++placeCounter;
            finalObj.countries[index]['states']['S' + stateCounter][
              'd' + districtsCounter
            ]['p' + placeCounter] = {};
            finalObj.countries[index]['states']['S' + stateCounter][
              'd' + districtsCounter
            ]['p' + placeCounter]['placeName'] = fp.name;
          });
        });
      });
    });
    console.log(finalObj);
  }

  state: any[] = [];
  district: any[] = [];
  place: any[] = [];

  showState: boolean = false;
  showDistrict: boolean = false;
  showPlace: boolean = false;

  selectCountry(country: any) {
    this.state = this.secondLevelArr.filter((el) => {
      if (el.parentId == country) {
        return true;
      } else {
        return false;
      }
    });
    this.showState = true;
  }

  selectState(state: any) {
    this.district = this.thirdLevelArr.filter((el) => {
      if (el.parentId == state) {
        return true;
      } else {
        return false;
      }
    });
    this.showDistrict = true;
  }

  // using one line arrow function
  selectDistrict(district: any) {
    this.place = this.fourthLevelArr.filter((el) => el.parentId == district);
    this.showPlace = true;
  }
}
